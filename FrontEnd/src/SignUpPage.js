...
const PhotoSlider = ({ photos }) => {
  ...
  return (
    <div>
      <img className=" w-5/6 rounded-2xl  h-48" src={photos[currentPhotoIndex]} alt={`Photo ${currentPhotoIndex + 1}`} />
    </div>
  );
};
...
function SignUp() {
  ...
  function handleRegisterUser(e) {
    ...
    // console.log(data);
    axios
      .post("http://ethnus-project-backend-3ou9.onrender.com/api/users", data)
      .then((res) => {
        // console.log(res);
        navigate("/");
      })
